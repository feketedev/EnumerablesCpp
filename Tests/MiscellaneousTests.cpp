...
namespace EnumerableTests {
	...
	static void CastConvertElements()
	{
		...
		// Implicit conversion and static_cast operations are no-op for exactly matching type:
		{
			...
			static_assert (std::is_same<decltype(integers), decltype(shortcut1)>(), "Unnecessary conversion!");
			static_assert (std::is_same<decltype(integers), decltype(shortcut2)>(), "Unnecessary conversion!");
